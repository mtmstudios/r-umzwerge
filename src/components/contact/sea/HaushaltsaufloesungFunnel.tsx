...
import { ArrowLeft, ArrowRight, Send, CheckCircle, X, Home, Building2, Users, Heart, ShieldCheck, Package, MapPin, Calendar, User, Phone, Mail, MessageSquare } from "lucide-react";
...
export function HaushaltsaufloesungFunnel({ open, onOpenChange }: HaushaltsaufloesungFunnelProps) {
  ...
  const validateStep = (): boolean => {
    switch (currentStep) {
      ...
      case 2:
        if (!formData.grund) {
          toast({ title: "Bitte wählen Sie den Grund", variant: "destructive" });
          return false;
        }
        return true;
      case 3:
        ...
        return true;
      case 4:
        if (!formData.plz || !formData.ort || !formData.zeitrahmen) {
          ...
          >
            <div className="text-center mb-3 lg:mb-8">
              ...
              <p className="text-sm lg:text-base text-muted-foreground">
                Wir begleiten Sie einfühlsam durch diesen Prozess
              </p>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 lg:gap-4">
              {objektarten.map((type) => (
                <button
                  key={type.id}
                  onClick={() => updateFormData("objektart", type.id)}
                  className={`group relative p-3 lg:p-6 rounded-xl border-2 transition-all duration-300 ${
                ...
                >
                  <div className="flex flex-col items-center gap-2 lg:gap-3">
                    <div className={`w-10 h-10 lg:w-14 lg:h-14 rounded-full flex items-center justify-center transition-colors ${
                      ...
                      <type.icon className="w-5 h-5 lg:w-7 lg:h-7" />
                    </div>
                    <span className="font-medium text-foreground text-sm lg:text-base">{type.label}</span>
                  </div>
                  {formData.objektart === type.id && (
                    <motion.div
                      layoutId="objektCheck"
                      className="absolute top-1 right-1 lg:top-2 lg:right-2 w-5 h-5 lg:w-6 lg:h-6 rounded-full bg-primary flex items-center justify-center"
                    >
        ...
        return (
          ...
          >
            <div className="text-center mb-3 lg:mb-8">
              ...
              <p className="text-sm lg:text-base text-muted-foreground">
                Damit wir uns optimal auf Ihre Situation einstellen können
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 gap-2 lg:gap-3">
              {gruende.map((grund) => (
                <button
                  key={grund.id}
                  onClick={() => updateFormData("grund", grund.id)}
                  className={`group relative p-3 lg:p-4 rounded-xl border-2 transition-all duration-300 text-left ${
                ...
                >
                  <div className="flex items-center gap-3 lg:gap-4">
                    <div className={`w-10 h-10 lg:w-12 lg:h-12 rounded-full flex items-center justify-center flex-shrink-0 transition-colors ${
                      ...
                      <grund.icon className="w-5 h-5 lg:w-6 lg:h-6" />
                    </div>
      ...
      case 3:
        return (
          ...
          >
            <div className="text-center mb-3 lg:mb-8">
              <h3 className="text-lg lg:text-2xl font-bold text-foreground mb-1 lg:mb-2">
                Welchen Umfang hat die Auflösung?
              </h3>
              <p className="text-sm lg:text-base text-muted-foreground">
            ...
            <div className="grid grid-cols-1 gap-2 lg:gap-3">
              {umfaenge.map((umfang) => (
                <button
                  key={umfang.id}
                  onClick={() => updateFormData("umfang", umfang.id)}
                  className={`group relative p-3 lg:p-5 rounded-xl border-2 transition-all duration-300 text-left ${
                    formData.umfang === umfang.id
                      ? "border-primary bg-primary/5"
                      : "border-border hover:border-primary/50 bg-card"
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <div className={`w-4 h-4 lg:w-5 lg:h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-colors ${
                      formData.umfang === umfang.id
                        ? "border-primary bg-primary"
                        : "border-muted-foreground"
                    }`}>
                      {formData.umfang === umfang.id && (
                        <div className="w-2 h-2 lg:w-2.5 lg:h-2.5 rounded-full bg-primary-foreground" />
                      )}
                    </div>
        ...
        return (
          ...
          >
            <div className="text-center mb-3 lg:mb-8">
              ...
              <p className="text-sm lg:text-base text-muted-foreground">
                Den genauen Termin besprechen wir später – ganz ohne Druck
              </p>
            </div>
            ...
            <div className="max-w-md mx-auto space-y-4">
              ...
              <div>
                <Label className="text-sm flex items-center gap-1.5 mb-2">
                  <Calendar className="w-3.5 h-3.5" /> Wann soll es stattfinden? *
                </Label>
                <div className="flex flex-wrap gap-2">
                  {zeitrahmenOptionen.map((option) => (
                    <button
                      key={option.id}
                      onClick={() => updateFormData("zeitrahmen", option.id)}
                      className={`px-3 py-2 rounded-full border-2 font-medium transition-all duration-300 text-sm ${
        ...
        return (
          ...
          >
            <div className="text-center mb-3 lg:mb-8">
              ...
              <p className="text-sm lg:text-base text-muted-foreground">
                Wir melden uns einfühlsam und zeitnah bei Ihnen
              </p>
            </div>
            ...
            <div className="max-w-md mx-auto space-y-3">
              ...
              <div>
                <Label htmlFor="nachricht" className="text-sm flex items-center gap-1.5">
                  <MessageSquare className="w-3.5 h-3.5" /> Nachricht (optional)
                </Label>
                <Textarea
                  id="nachricht"
                  ...
                  className="mt-1 min-h-[60px] lg:min-h-[80px]"
                />
              </div>
            </div>
          </motion.div>
  ...
  const renderSuccessState = () => (
    ...
    >
      ...
      <p className="text-sm md:text-base text-muted-foreground">
        Wir melden uns einfühlsam und zeitnah bei Ihnen – in der Regel innerhalb von 24 Stunden.
      </p>
    </motion.div>
  );
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto p-6 md:p-8">
        <VisuallyHidden>
          <DialogTitle>Haushaltsauflösung anfragen</DialogTitle>
        </VisuallyHidden>
        {content}
      </DialogContent>
    </Dialog>
