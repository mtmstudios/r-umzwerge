...
export function FlipCard({ problem, solution, index, isGentle, isMobile, isVisible }: FlipCardProps) {
  ...
  return (
    <div
      ...
      style={!isMobile ? { transitionDelay: isVisible ? `${index * 150}ms` : '0ms' } : undefined}
    >
      ...
      <div className="mx-5 mt-4 mb-5 px-4 py-3 border-l-[3px] border-accent bg-accent/5 rounded-r-lg">
        <div className="flex items-start gap-2">
          ...
          <p className="text-base lg:text-lg text-foreground font-medium leading-relaxed">{solution}</p>
        </div>
      </div>
    </div>
  );
